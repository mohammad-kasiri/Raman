@extends('admin.layout.master' , ['title' => 'افزودن نقش جدید'])
...
@section('subheader')
    @can('roles.index')
        ...
        @endphp
    @endcan
    <x-dashboard.subheader :links='$buttons ?? []' :title="'افزودن نقش جدید'" />
@endsection
            ...
            <div class="card card-custom mb-4">
                ...
                <div class="card-body">
                    <div class="row justify-content-center">
                        <div class="col-md-8">
                            <x-dashboard.form.row-input     label="عنوان"     name="label" />
                            <x-dashboard.form.text.textarea label="توضیحات"  name="description" rows="2"/>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    @foreach($permissions as $permission)
                        <x-dashboard.form.checkbox
                            ...
                            value="{{ $permission->id }}"
                            checked="{{old('permissions.'.$permission->id)  ? 1 : 0}}"
                        >
                            {{$permission->label}}
                        </x-dashboard.form.checkbox>
                        <div class="row my-5">
                            @foreach($permission->children as $permit)
                                <div class="col-md-4 my-1">
                                    <x-dashboard.form.checkbox
                                        ...
                                        value="{{ $permit->id }}"
                                        checked="{{old('permissions.'.$permit->id) ? 1 : 0}}"
                                    >
                                        {{ $permit->label }}
                                    </x-dashboard.form.checkbox>
                                </div>
